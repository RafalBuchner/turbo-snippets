...
from misc import getDefault, getFontDefault, getColorDefault, SnippetCreatorError, nsStringLength
...
class CodeEditor(TextEditor):
    ...
    def insertVarCallback(self, sender):
        # self.getNSTextView().selectedRanges()
        self.getNSTextView().insertText_('$VAR$')

    def _setContextualMenu(self):
        y,x,p = (10,10,10)
        btnMenuItem = AppKit.NSMenuItem.alloc().initWithTitle_action_keyEquivalent_("btnMenu", '', '')
        group = Group((0,0,17+2*p,100))
        button =Button((x, y, -p, 17),'insert variable',callback=self.insertVarCallback,sizeStyle='small')
        group.insertVarBtn = button
        self.rightClickMenuObjs = [group.insertVarBtn]

        # Building NSMenu
        view = group.getNSView()
        btnMenuItem.setView_(view)

        # builder = MenuBuilder([
        #      btnMenuItem,
        #  ])
        self.menu = AppKit.NSMenu.alloc().init()
        self.menu.addItem_(btnMenuItem)

        # self.menu = builder.getMenu()
        self.menu.setMinimumWidth_(100)
        self.menu.setAutoenablesItems_(False)
        view.setFrame_(((0, 0), (100, 17+2*p)))
        self.getNSTextView().rightMenu = self.menu
